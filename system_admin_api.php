...
try {
    // ============================================================
    // 1. DATA ENTRY INTERFACE — create_branch, create_manager
    ...
    if ($action === 'create_branch') {
        ...
        if (empty($name) || empty($city)) { echo json_encode(['success' => false, 'message' => 'Branch name and city are required.']); exit; }
        ...
        } catch (Exception $fbErr) { error_log('Firebase sync error (create_branch): ' . $fbErr->getMessage()); }
    ...
    elseif ($action === 'create_manager') {
        ...
        if (empty($fname) || empty($email) || empty($branch_id)) { echo json_encode(['success' => false, 'message' => 'All fields required.']); exit; }

        $stmt = $pdo->prepare("SELECT COUNT(*) FROM BRANCH_MANAGER WHERE Mgr_Brnch_ID = ?");
        $stmt->execute([$branch_id]);
        if ($stmt->fetchColumn() > 0) { echo json_encode(['success' => false, 'message' => 'This branch already has a manager.']); exit; }
        ...
        foreach ($tables as $t) {
            ...
            $stmt->execute([$email]);
            if ($stmt->fetchColumn() > 0) { echo json_encode(['success' => false, 'message' => 'A user with this email already exists.']); exit; }
        }
        ...
        // Firebase DATA ENTRY
        try {
            fbAdd($firestore, 'branch_manager', [
                'mysql_id'   => (string)$new_id,
        ...
        } catch (Exception $fbErr) { error_log('Firebase sync error (create_manager): ' . $fbErr->getMessage()); }

        echo json_encode(['success' => true, 'message' => 'Manager account created!']);
    }

    // ============================================================
    // 2. DISPLAY INTERFACE — get_branches, get_menu, get_all_users, get_stats
    ...
    elseif ($action === 'get_branches') {
        ...
        try {
            ...
            foreach ($docs as $doc) {
                if ($doc->exists()) {
                    $fb_branches[] = array_merge(['id' => $doc->id()], $doc->data());
                }
            }
            if (!empty($fb_branches)) {
                echo json_encode(['success' => true, 'data' => $fb_branches, 'source' => 'firebase']);
                exit;
            }
        } catch (Exception $fbErr) { error_log('Firebase fetch error (get_branches): ' . $fbErr->getMessage()); }

        // Fallback MySQL
        $stmt = $pdo->query("SELECT b.*, m.Mgr_FName as fname, m.Mgr_LName as lname FROM BRANCH b LEFT JOIN BRANCH_MANAGER m ON b.Brnch_ID = m.Mgr_Brnch_ID");
    ...
    elseif ($action === 'get_menu') {
        ...
        } catch (Exception $fbErr) { error_log('Firebase fetch error (get_menu): ' . $fbErr->getMessage()); }

        // Fallback MySQL
        $stmt = $pdo->query("SELECT m.*, b.Brnch_Name as Creator_Branch_Name FROM MENU_ITEM m LEFT JOIN BRANCH b ON m.Menu_Brnch_ID = b.Brnch_ID WHERE m.Menu_Status != 'D'");
        echo json_encode(['success' => true, 'data' => $stmt->fetchAll(), 'source' => 'mysql']);
    }
    elseif ($action === 'get_managers') {
        $stmt = $pdo->query("SELECT m.Mgr_ID as id, m.Mgr_Brnch_ID, m.Mgr_FName as fname, m.Mgr_LName as lname, m.Mgr_Email as email, m.Mgr_MobileNum as mobile, m.Mgr_Status as status, b.Brnch_Name FROM BRANCH_MANAGER m LEFT JOIN BRANCH b ON m.Mgr_Brnch_ID = b.Brnch_ID");
        echo json_encode(['success' => true, 'data' => $stmt->fetchAll()]);
    }
    elseif ($action === 'get_staff') {
        $stmt = $pdo->query("SELECT s.Staff_ID as id, s.Staff_Brnch_ID, s.Staff_Mgr_ID, s.Staff_FName as fname, s.Staff_LName as lname, s.Staff_Email as email, s.Staff_MobileNum as mobile, s.Staff_Role as role, s.Staff_Status as status, b.Brnch_Name, m.Mgr_FName, m.Mgr_LName FROM STAFF s LEFT JOIN BRANCH b ON s.Staff_Brnch_ID = b.Brnch_ID LEFT JOIN BRANCH_MANAGER m ON s.Staff_Mgr_ID = m.Mgr_ID WHERE s.Staff_Role = 'Kitchen Staff'");
        echo json_encode(['success' => true, 'data' => $stmt->fetchAll()]);
    }
    elseif ($action === 'get_riders') {
        $stmt = $pdo->query("SELECT r.Rider_ID as id, r.Rider_Brnch_ID, r.Rider_FName as fname, r.Rider_LName as lname, r.Rider_Email as email, r.Rider_MobileNum as mobile, r.Rider_Status as status, b.Brnch_Name FROM RIDER r LEFT JOIN BRANCH b ON r.Rider_Brnch_ID = b.Brnch_ID");
        echo json_encode(['success' => true, 'data' => $stmt->fetchAll()]);
    }
    ...
    elseif ($action === 'get_stats') {
        ...
        echo json_encode(['success' => true, 'stats' => ['branches' => $branchCount, 'managers' => $managerCount, 'staff' => $staffCount, 'riders' => $riderCount]]);
    }
    elseif ($action === 'get_reports') {
        ...
        $topItems    = $pdo->query("SELECT m.Menu_Name, SUM(oi.OItem_Quantity) as total_qty FROM ORDER_ITEM oi JOIN MENU_ITEM m ON oi.OItem_Menu_ID = m.Menu_ID GROUP BY m.Menu_ID ORDER BY total_qty DESC LIMIT 5")->fetchAll();
        echo json_encode(['success' => true, 'data' => ['dailySales' => $dailySales, 'totalOrders' => $totalOrders, 'topItems' => $topItems]]);
    }

    // ============================================================
    // 3. UPDATE/DELETE INTERFACE — update_branch, update_manager,
    //    update_menu, update_staff, update_rider, delete_*
    ...
    elseif ($action === 'update_branch') {
        ...
        if (!$id || empty($name) || empty($city)) { echo json_encode(['success' => false, 'message' => 'ID, name, and city required.']); exit; }
    ...
    elseif ($action === 'update_manager') {
        ...
        } catch (Exception $fbErr) { error_log('Firebase sync error (update_manager): ' . $fbErr->getMessage()); }
    ...
    elseif ($action === 'update_manager_status') {
        ...
        } catch (Exception $fbErr) { error_log('Firebase sync error (update_manager_status): ' . $fbErr->getMessage()); }
    ...
    elseif ($action === 'update_menu') {
        ...
        if (!$id || empty($name) || empty($cat)) { echo json_encode(['success' => false, 'message' => 'ID, name, and category required.']); exit; }
        if (!empty($image)) $image = saveMenuImageLocal($image, $id);
    ...
    elseif ($action === 'update_menu_status') {
        ...
        } catch (Exception $fbErr) { error_log('Firebase sync error (update_menu_status): ' . $fbErr->getMessage()); }
    ...
    elseif ($action === 'delete_branch') {
        $id = $data['id'] ?? null;
        if (!$id) { echo json_encode(['success' => false, 'message' => 'Branch ID required.']); exit; }
        ...
        } catch (Exception $fbErr) { error_log('Firebase sync error (delete_branch): ' . $fbErr->getMessage()); }

        echo json_encode(['success' => true, 'message' => 'Branch deleted successfully!']);
    }
    elseif ($action === 'delete_manager') {
        $id = $data['id'] ?? null;
        if (!$id) { echo json_encode(['success' => false, 'message' => 'Manager ID required.']); exit; }
        ...
        } catch (Exception $fbErr) { error_log('Firebase sync error (delete_manager): ' . $fbErr->getMessage()); }
    ...
    elseif ($action === 'delete_user') {
        $id     = $data['id'] ?? null;
        $source = $data['source'] ?? '';
        if (!$id || !$source) { echo json_encode(['success' => false, 'message' => 'ID and source required.']); exit; }
        ...
        if (!isset($tableMap[$source])) { echo json_encode(['success' => false, 'message' => 'Invalid source.']); exit; }
        if ($source === 'Admin' && $id == $_SESSION['user_id']) { echo json_encode(['success' => false, 'message' => 'You cannot delete your own account.']); exit; }
        ...
        } catch (Exception $fbErr) { error_log('Firebase sync error (delete_user): ' . $fbErr->getMessage()); }

        echo json_encode(['success' => true, 'message' => 'User deleted successfully!']);
    }

    // ============================================================
    // 4. TRANSACTION INTERFACE — create_manager (multi-collection)
    //    Validates across 5 tables + inserts into branch_manager atomically
    //    Already handled above in create_manager action
    // ============================================================
    elseif ($action === 'create_menu') {
        echo json_encode(['success' => false, 'message' => 'Unauthorized. Only branch managers can add menu items.']);
        exit;
    }
